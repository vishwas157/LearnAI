...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [analyticsRes, materialsRes, quizRes] = await Promise.all([
          analyticsService.getAnalytics(),
          materialService.getMaterials(),
          quizService.getQuizzes(),
        ]);

        if (analyticsRes.success) setAnalytics(analyticsRes.data.analytics);
  ...
  if (loading) {
    return <LoadingSpinner message="Loading your student dashboard..." />;
  }

  // Filter materials that are in progress or recently opened
  const inProgressMaterials = materials.slice(0, 3);
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          {t('dashboard.subtitle', 'Continue your learning journey.')}
        </p>
      </div>

      {/* 2. CONTINUE LEARNING (Top Priority Section) */}
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          >
            <span>{t('dashboard.viewAll', 'View All Materials')}</span>
        ...
        {inProgressMaterials.length > 0 ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1 mt-0.5">
                        {mat.description || 'Study document'}
                      </p>
                    </div>
          ...
          <Card className="p-6 text-center">
            ...
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">No materials uploaded yet</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mx-auto mt-1 mb-3">
              Upload your lecture notes, PDFs, or study guides to start reading and listening.
            </p>
            <Link to="/materials">
      ...
      {/* 3. YOUR PROGRESS (3-4 Compact Non-Oversized Statistics) */}
      <section className="space-y-3">
        <h2 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
          <Card className="p-3.5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                {t('dashboard.materialsStudied', 'Materials Studied')}
              </p>
            </div>
          </Card>
          ...
          <Card className="p-3.5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                {t('dashboard.quizzesAttempted', 'Quizzes Completed')}
              </p>
            </div>
          </Card>
          ...
          <Card className="p-3.5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
                {t('dashboard.averageScore', 'Average Score')}
              </p>
            </div>
          </Card>
          ...
          <Card className="p-3.5 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold text-slate-900 dark:text-white leading-tight">
                {analytics?.studyStreakDays || 1} {t('dashboard.days', 'Days')}
              </p>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">
      ...
      {/* 4. RECENT ACTIVITY & RECOMMENDED FOR YOU (2-Column Grid) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Link to="/analytics" className="text-xs text-slate-500 hover:text-slate-800 dark:hover:text-slate-200">
              View Analytics →
            </Link>
          </div>
          ...
          <Card className="p-0 overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
            {recentActivities.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-7 h-7 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center shrink-0">
                      {act.activityType === 'quiz_attempt' ? (
                        <Award className="w-3.5 h-3.5 text-emerald-600" />
                      ) : act.activityType === 'read_material' ? (
                        <BookOpen className="w-3.5 h-3.5 text-blue-600" />
                      ) : act.activityType === 'ai_chat' ? (
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400">
                        {act.durationSeconds ? `${Math.round(act.durationSeconds / 60)} min session` : 'Study activity'}
                      </p>
                    </div>
                  </div>
              ...
              <div className="p-6 text-center text-xs text-slate-400">
                No recent study activity logged yet. Start reading a material or take a quiz!
              </div>
            )}
          </Card>
        </div>

        {/* Right 1 Col: RECOMMENDED FOR YOU */}
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {recommendedQuizzes.map((q) => (
              <Card key={q._id} className="p-4 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[10px] text-slate-400 uppercase font-medium">{q.difficulty}</span>
                  </div>
                  <h4 className="text-xs font-semibold text-slate-900 dark:text-white line-clamp-1">{q.title}</h4>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">
                    {q.questions?.length || 5} questions to test your knowledge
                  </p>
                </div>

                <div className="pt-3 mt-3 border-t border-slate-100 dark:border-slate-800">
                  <Link
                    to={`/quiz/${q._id}/attempt`}
            ...
            <Card className="p-4 bg-gradient-to-br from-brand-50/50 to-indigo-50/30 dark:from-slate-900 dark:to-slate-900 border-brand-200/60 dark:border-slate-800">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-xs font-bold text-slate-900 dark:text-white">Need help understanding?</h4>
              </div>
              <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed mb-3">
                Ask your AI Tutor to explain complex topics, summarize papers, or generate revision notes.
              </p>
              <Link to="/tutor">
